...
# Using the pre-trained model, initializing the model
resnet_model = models.resnet18(weights=models.ResNet18_Weights.DEFAULT)

# Replacing final layer to match our number of classes
...
num_epochs = 50
best_val_acc = 0.0
resnet_log_file = open("Output/ResNet-18/resnet_training_log.txt", "w")
...
for epoch in tqdm(range(1, num_epochs+1)):
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(resnet_model.state_dict(), f"Output/ResNet-18/best_resnet_model.pth")
    ...
    print(log_msg)
    resnet_log_file.write(log_msg + "\n")

resnet_log_file.close()
print("ResNet training complete. Best Val Acc: {:.4f}".format(best_val_acc))

## Evaluation

# Load best model weights
resnet_model.load_state_dict(torch.load("Output/ResNet-18/best_resnet_model.pth"))
resnet_model.eval()
...
pred_df = pd.DataFrame({"image": all_fnames, "label": pred_labels_text})
pred_df.to_csv("Output/ResNet-18/resnet_test_predictions.csv", index=False)
print("Saved -> Output/ResNet-18/resnet_test_predictions.csv")
...
# Save a text report
with open("Output/ResNet-18/resnet_test_eval.txt", "w") as f:
...
print(report)
print("Saved -> Output/ResNet-18/resnet_test_eval.txt")
...
def plot_cm(matrix, labels, title, outpath):
    ...
    for i in range(matrix.shape[0]):
        for j in range(matrix.shape[1]):
            ax.text(j, i, f"{matrix[i, j]:.2f}" if matrix.dtype.kind == 'f' else f"{matrix[i, j]}",
                    ha="center", va="center",
...
plot_cm(cm, class_names, "ResNet-18 Confusion Matrix", "Output/ResNet-18/resnet_confusion_matrix.png")
plot_cm(cm_norm, class_names, "ResNet-18 Confusion Matrix (Normalized)", "Output/ResNet-18/resnet_confusion_matrix_normalized.png")
print("Saved CM images in Output/ResNet-18/")